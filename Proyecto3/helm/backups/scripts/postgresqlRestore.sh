...
az storage blob download --container $CONTAINER --name postgresql/$BACKUP_NAME/$ARCHIVE_NAME --file postgresqlrestore/$BACKUP_NAME --auth-mode key --connection-string $CONNECTION_STRING_AZURE

# Restore a PostgreSQL database from a backup
PGPASSWORD="$POSTGRESQL_PASSWORD" pg_restore --host $DB_HOST -U $POSTGRESQL_USERNAME --clean --if-exists postgresqlrestore/$BACKUP_NAME